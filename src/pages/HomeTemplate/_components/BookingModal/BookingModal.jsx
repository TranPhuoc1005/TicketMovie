...
export default function BookingModal(props) {
    const { isOpen, onClose, cinemaDetail, refetchBookingHistory } = props;
    ...
    const { mutateAsync: bookTicket, isPending: isBooking } = useMutation({
        mutationFn: (bookingData) => bookingTicketApi(bookingData),
    });
    ...
    const handleBookTicket = async () => {
        ...
        const bookingData = {
            maLichChieu: selectedSchedule?.maLichChieu,
        ...
        if(!bookingData.maLichChieu || bookingData.danhSachVe.length === 0) {
            alert('Vui lòng chọn suất chiếu và ghế trước khi đặt');
            return;
        }
        try {
            await bookTicket(bookingData);
            await refetchBookingHistory?.();
    ...
    return (
        <section id="bookingModal" className="fixed inset-0 z-50 overflow-y-auto">
            <div
                className="fixed inset-0 bg-black/70 backdrop-blur-sm transition-opacity duration-300"
                onClick={onClose}
            />
            <div className="flex min-h-full items-center justify-center p-4">
                <div className="relative mx-auto glass-effect rounded-3xl w-full max-w-5xl max-h-[90vh] overflow-y-auto border border-slate-700/50">
                    <div className="p-8">
                        ...
                        <div className={`booking-step ${currentStep === 3 ? '' : 'hidden'}`} id="step3">
                            ...
                            <div className="flex space-x-4 mt-8">
                                ...
                                <button 
                                    ...
                                    disabled={isBooking}
                                    className="flex-1 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white py-4 rounded-xl font-semibold text-lg transition-all hover:scale-[1.02] hover:shadow-lg flex items-center justify-center space-x-2 cursor-pointer"
                                >
                                    {isBooking ? 'Đang xử lý...' : (
                                        <>
